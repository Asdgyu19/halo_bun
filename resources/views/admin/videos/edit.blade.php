...
<div class="container mx-auto px-4">
    <h1 class="text-2xl font-bold text-[var(--blue)] mb-6">Edit Video Edukasi</h1>
    ...
    <form method="POST" action="{{ route('admin.videos.update', $video) }}" enctype="multipart/form-data" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <select name="video_type" id="video_type" class="w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:border-[var(--blue)]" required>
                    ...
                    <option value="tiktok" {{ old('video_type', $video->video_type) == 'tiktok' ? 'selected' : '' }}>TikTok</option>
        ...
        <div class="mb-4 p-4 bg-gray-50 rounded-lg">
            ...
            <div class="aspect-w-16 aspect-h-9 bg-gray-200 rounded-lg overflow-hidden max-w-md">
                ...
                @elseif($video->video_type === 'tiktok')
                    <div class="flex items-center justify-center h-full">
                        <a href="{{ $video->video_url }}" target="_blank" class="text-[var(--blue)] hover:underline">
                            <i class="fab fa-tiktok text-4xl"></i>
                            <p class="mt-2">Lihat di TikTok</p>
                        </a>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <select name="trimester" id="trimester" class="w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:border-[var(--blue)]">
                    ...
                    <option value="1" {{ old('trimester', $video->trimester) == '1' ? 'selected' : '' }}>Trimester 1</option>
                    <option value="2" {{ old('trimester', $video->trimester) == '2' ? 'selected' : '' }}>Trimester 2</option>
                    <option value="3" {{ old('trimester', $video->trimester) == '3' ? 'selected' : '' }}>Trimester 3</option>
                </select>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <div class="mt-2">
                    ...
                    <img src="{{ Storage::url($video->thumbnail) }}" alt="Current thumbnail" class="w-24 h-16 object-cover rounded">
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="mb-4">
            ...
            <input 
                ...
                type="number"
                placeholder="0" 
                value="{{ old('view_count', $video->view_count) }}"
                class="w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:border-[var(--blue)]" 
                min="0"
            />
            <p class="text-sm text-gray-500 mt-1">Jumlah views saat ini: {{ number_format($video->view_count) }}</p>
        </div>
